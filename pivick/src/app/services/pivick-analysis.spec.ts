      ...
      cubes: [
        {
          ...
          dimensions: [
            ...
            { name: 'price', title: 'Price', type: 'number', shortTitle: 'Price' },
          ],
          measures: [
            { name: 'count', title: 'Count', type: 'number', shortTitle: 'Count' },
          ],
        },
      ],
    } as unknown as Meta);
...
describe('PivickAnalysis', () => {
  ...
  describe('loadSchema', () => {
    ...
    it('should handle errors when loading the schema', (done) => {
      spyOn(cubeClient, 'meta').and.returnValue(throwError(() => new Error('Error loading schema')));
