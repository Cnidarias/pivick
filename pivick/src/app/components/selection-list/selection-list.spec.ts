...
class MockPivickAnalysis {
  ...
  getDimensionByName = (name: string) => ({ name, shortTitle: name } as TCubeDimension);
  getMeasureByName = (name: string) => ({ name, shortTitle: name } as TCubeMeasure);
...
describe('SelectionList', () => {
  ...
  it('should get label for a dimension', () => {
    const label = component.getLabel({ name: 'test', shortTitle: 'Test' } as TCubeDimension);
